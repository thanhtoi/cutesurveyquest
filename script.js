...
// Default Questions Set (Combining 4 original + 5 new questions)
...
// Auto-load questions from sample_questions.xlsx or fallback to defaults
async function loadExcelQuestionsOrDefault() {
  try {
    const response = await fetch('sample_questions.xlsx');
    if (response.ok) {
      ...
      let loaded = parseExcelRows(rows);
      if (loaded.length > 0) {
        setQuestions(loaded);
        return;
      }
...
// Parse Excel Rows (Question text + up to 4 options, auto add custom option if < 4)
function parseExcelRows(rows) {
  let result = [];
  rows.forEach((row, idx) => {
    if (!row || row.length === 0) return;
    
    // Skip header if contains 'câu hỏi'
    if (idx === 0 && row[0].toString().toLowerCase().includes('câu hỏi')) return;
